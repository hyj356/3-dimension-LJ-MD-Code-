...
program main
  ...
  real(wp) :: start, ends
  integer(si) :: moreCycles, nMol, stepAvg, stepCount, stepEquil, stepLimit, nx, ny, nz, i   !< 仿真用到的参数


  call cpu_time(start)
  !! 从外部文件中读取初始化变量的具体数值
  call read_file(deltaT, density, nx, ny, nz, stepAvg, stepEquil, stepLimit, temperature, InitUcell)
  ...
  moreCycles = 1
  write(stdout, '(*(A, 5x))') 'step', 'Temp', 'Total', 'KE', 'UE', 'Vsum', 'Pressure'
