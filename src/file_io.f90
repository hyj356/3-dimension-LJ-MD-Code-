module file_io
  ...
  use vector, only: VecI, Mol, VecR, Sum, SumVec2
  implicit none
  private
  public :: read_file, write_property, Dump_xyz
contains
  subroutine read_file(deltaT, density, nx, ny, nz, stepAvg, stepEquil, stepLimit, temperature, InitUcell)
    !! 按照fortran的namelist格式从外部文件读取仿真参数
    integer(si), intent(inout) :: nx, ny, nz                            !< 在x, y方向上扩展的单胞数
    ...
    namelist /InputVar/ deltaT, density, nx, ny, nz, stepAvg, stepEquil, stepLimit, temperature   !< 以namelist的方式读取
  ...
  subroutine write_property(stepCount, Atom, vSum, uSum, virsum, density)
    ...
    real(wp) :: temperature                           !< 系统的温度

    nMoli = 1.d0 / size(Atom)         !! 为了尽量避免除法, 需要多次用到的分母要提前准备好
    vvsum = sum(Atom%rv%VLensq())     !! 求取

    vSum = sum(Atom%rv)     !! 计算系统的速度分量
    sumV = sum(vSum) * nMoli                        !! 计算系统的总动量
    kinEnergy = 0.5d0 * SumVec2(Atom%rv) * nMoli    !! 计算平均动能
    totEnergy = kinEnergy + uSum*nMoli                                            !! 计算平均总能量
    pressure = density * (vvsum + virsum) /(size(Atom) * NDIM)
    temperature = kinEnergy * 2.d0 / NDIM
    write(stdout, "(I0, 3x, *(f0.7, 3x))") stepCount, temperature, totEnergy, kinEnergy, uSum*nMoli, sumV, pressure
  ...
  subroutine Dump_xyz(Atom, prefix, frame)
    ...
    fname = prefix // trim(suffix)        !! 这里将字符串类型的帧数与文件后缀名相加
    ...
    do i = 1, nMol                        !! 循环将原子坐标写入文本文件之中
      write(dumpid, '(A, E24.15, 2X, E24.15, 2X, E24.15)') 'Ar', Atom(i)%r%x, Atom(i)%r%y, Atom(i)%r%z
    end do
    ! write(dumpid, '(A)') 'ITEM: TIMESTEP'
    ! write(dumpid, '(I0)') frame
    ! write(dumpid, '(A)') 'ITEM: NUMBER OF ATOMS'
    ! write(dumpid, '(I0)') nMol
    ! write(dumpid, '(A)') 'ITEM: BOX BOUNDS pp pp pp'
    ! write(dumpid, '(G0, 1X, G0)') -7.82524d0, 7.82524d0
    ! write(dumpid, '(G0, 1X, G0)') -7.82524d0, 7.82524d0
    ! write(dumpid, '(G0, 1X, G0)') -7.82524d0, 7.82524d0
    ! write(dumpid, '(A)') 'ITEM: ATOMS id type x y z'
    ! do i = 1, nMol
    !   write(dumpid, '(I0, 2x, I0, E24.15, 2X, E24.15, 2X, E24.15)') i, 1, Atom(i)%r%x, Atom(i)%r%y, Atom(i)%r%z
    ! end do
    close(dumpid)
