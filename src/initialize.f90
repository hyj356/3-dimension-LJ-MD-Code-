module initialize
  use global_var, only: wp, NDIM
  use vector, only: Mol, VecR, VecI, sum
  implicit none
...
contains
  ...
  subroutine InitCoord(Atom, InitUcell, region)
    ...
    type(VecR), intent(in) ::  region                 !< 分别对应一个单胞的xy长度和盒子的xy长度
    type(VecI), intent(in) :: InitUcell               !< 在x和y方向上的单胞个数
    integer :: n, nx, ny, nz                          !< 一共有n个单胞, x方向上有nx个, y方向上有ny个, n = nx * ny
    type(VecR) :: c, gap                              !< 计算用到的临时变量

    call gap%VDiv(region, InitUcell)
    n = 1

    !! 双层循环放置原子
    do nz = 0, InitUcell%z - 1
      do ny = 0, InitUcell%y - 1
        do nx = 0, InitUcell%x - 1
          c%x = nx + 0.5d0;   c%y = ny + 0.5d0; c%z = nz + 0.5d0
          call c%VMul(c, gap)
  ...
  pure subroutine InitAccels(Atom)
    !! 这里只是����������������的将所有的原子的加速度都初始化为0
    type(Mol), intent(inout), dimension(:) :: Atom    !< 传入的原子集合信息, 包括位置, 速度, ����速度
  ...
  pure subroutine SetParams(rCut, region, density, initUcell, nMol, velMag, temperature)
    ...
    !integer, parameter :: NDIM = 2            !! 模拟在三维环境下进行

    rCut = 2.0 ** (1./6)                      !! 计算截断半径
    region%x = initUcell%x / sqrt(density)
    region%y = initUcell%y / sqrt(density)
    region%z = initUcell%z / sqrt(density)
    !nMol = initUcell%x * initUcell%y * initUcell%z              !! 计算原子个数
    nMol = initUcell%VIProd()                 !! 计算原子个数
